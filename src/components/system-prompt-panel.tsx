...
export function SystemPromptPanel(): React.ReactNode {
  ...
  return (
    <>
      {!open && (
        <button type="button" onClick={() => setOpen(true)} className="fixed right-14 top-3 z-40 flex h-10 w-10 items-center justify-center rounded-full text-neutral-700 transition hover:bg-neutral-100 dark:text-neutral-200 dark:hover:bg-neutral-800" aria-label="Abrir configurações do agente" title="Configurações do agente">
          ...
          <div className="flex max-h-[92vh] w-full max-w-5xl flex-col rounded-t-2xl bg-white shadow-2xl sm:rounded-2xl dark:bg-neutral-950">
            <div className="flex items-start justify-between border-b border-neutral-200 px-5 py-4 dark:border-neutral-800">
              <div>
                ...
                <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Prompt, conhecimento e recursos do runtime.</p>
              </div>
                ...
                {tab === "prompt" ? (
                  <>
                    <div className="p-5">
                      ...
                      <p className="mt-1 mb-4 text-sm text-neutral-500 dark:text-neutral-400">Salvo no Assistant do LangGraph e aplicado aos próximos runs.</p>
                      ...
                      <textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} disabled={loading || saving} spellCheck={false} className="min-h-[48vh] w-full resize-y rounded-xl border border-neutral-300 bg-neutral-50 p-4 font-mono text-sm leading-6 text-neutral-900 outline-none dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100" />
                    ...
                    <div className="flex items-center justify-end gap-3 border-t border-neutral-200 px-5 py-4 dark:border-neutral-800">
                      ...
                      <button type="button" onClick={() => void savePrompt()} disabled={!isDirty || loading || saving || !assistant} className="rounded-lg bg-neutral-950 px-5 py-2 text-sm font-medium text-white hover:bg-neutral-800 disabled:cursor-not-allowed disabled:opacity-40 dark:bg-white dark:text-neutral-950">
