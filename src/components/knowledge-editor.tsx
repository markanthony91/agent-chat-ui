...
export function KnowledgeEditor(): React.ReactNode {
  ...
  const load = async () => {
    ...
    try {
      ...
      setSelected(null); setEditing(false); setDraft(""); setOpenFolders(new Set());
  ...
  return <div className="p-5">
    ...
    <div className="mt-5 grid min-h-[52vh] grid-cols-1 gap-4 md:grid-cols-[280px_1fr]">
      ...
      <div className="min-w-0 rounded-xl border"><div className="flex items-center justify-between gap-3 border-b px-4 py-3"><p className="truncate text-sm font-medium">{selected?.path ?? "Selecione um arquivo"}</p>{selected && !editing && <button onClick={() => { setDraft(selected.content ?? ""); setEditing(true); }} className="flex items-center gap-2 rounded-lg border px-3 py-1.5 text-sm"><Pencil className="h-4 w-4" />Editar</button>}{selected && editing && <div className="flex gap-2"><button onClick={() => { setDraft(selected.content ?? ""); setError(null); setEditing(false); }} className="flex items-center gap-2 rounded-lg border px-3 py-1.5 text-sm"><X className="h-4 w-4" />Cancelar</button><button onClick={() => void save()} disabled={saving || draft === (selected.content ?? "")} className="flex items-center gap-2 rounded-lg bg-neutral-950 px-3 py-1.5 text-sm text-white disabled:opacity-40 dark:bg-white dark:text-neutral-950"><Save className="h-4 w-4" />{saving ? "Salvando..." : "Salvar"}</button></div>}</div>{editing ? <textarea value={draft} onChange={(event) => setDraft(event.target.value)} spellCheck={false} className="min-h-[55vh] w-full resize-none bg-transparent p-4 font-mono text-xs leading-6 outline-none" /> : <pre className="max-h-[55vh] overflow-auto whitespace-pre-wrap break-words p-4 font-mono text-xs leading-6 text-neutral-700 dark:text-neutral-300">{loading && selected ? "Carregando..." : selected?.content ?? "Selecione um arquivo OKF para visualizar seu conteúdo."}</pre>}</div>
    </div>
  </div>;
}
