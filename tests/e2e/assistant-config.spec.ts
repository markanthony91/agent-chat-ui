...
import { resolveAssistant, saveAssistantContext } from "../../src/lib/assistant-config";

const id = "2c03ca0a-4481-4e9d-943f-9a1382e644be";
const managed = { assistant_id: id, graph_id: "agent", metadata: { zerai_default: true },
  context: { system_prompt: "Saved prompt", agent_instructions: "Keep rules", active_workflow: "Keep workflow" } };

test("resolver supports graph default, explicit UUID, missing and ambiguous configurations", async () => {
  let matches = [managed];
  let lookups = 0;
  const client = { assistants: {
    search: async () => { lookups++; return matches; },
    get: async (ref: string) => { expect(ref).toBe(id); return managed; },
  ...
  matches = [];
  await expect(resolveAssistant(client, "agent")).rejects.toThrow("não encontrado");
});

test("saving reads fresh context, preserves other settings, confirms read-back", async () => {
  let context = { ...managed.context };
  const client = { assistants: {
    get: async () => ({ ...managed, context }),
    update: async (_id: string, data: { context: typeof context }) => { context = data.context; },
  } } as unknown as Client;
  const saved = await saveAssistantContext(client, id, { system_prompt: "New prompt" });
  expect(saved.context).toEqual({ ...managed.context, system_prompt: "New prompt" });
});
...
test("system assistant cannot be edited and unconfirmed saves are errors", async () => {
  ...
  let writes = 0;
  const client = { assistants: {
    get: async () => ({ ...managed, metadata: system ? { created_by: "system" } : {} }),
    update: async () => { writes++; },
  } } as unknown as Client;
  await expect(saveAssistantContext(client, id, { system_prompt: "New" })).rejects.toThrow("persistente");
  expect(writes).toBe(0);
  system = false;
  await expect(saveAssistantContext(client, id, { system_prompt: "New" })).rejects.toThrow("confirmar");
});

test("system fallback can be read when no managed default exists", async () => {
  const client = { assistants: { search: async (query: { metadata?: unknown }) =>
    query.metadata ? [] : [{ ...managed, metadata: { created_by: "system" } }],
  } } as unknown as Client;
  expect((await resolveAssistant(client, "agent")).assistant_id).toBe(id);
});

for (const reference of ["agent", id]) {
  test(`prompt persists across reload; chat uses the edited assistant: ${reference}`, async ({ page }) => {
    ...
    await page.route("https://runtime.invalid/**", async (route) => {
      ...
      if (path === `/assistants/${id}`) {
        if (req.method() === "PATCH") {
          if (failSave) return route.fulfill({ status: 500, json: { detail: "Controlled save error" } });
          record = { ...record, context: body.context };
        }
        return route.fulfill({ json: record });
      }
      if (path.endsWith("/runs/stream")) {
        runIds.push(body.assistant_id);
        return route.fulfill({ contentType: "text/event-stream", body: 'event: end\ndata: {}\n\n' });
      }
      return route.fulfill({ json: path === "/threads" ? { thread_id: "synthetic-prompt-thread" } : [] });
    });
    ...
    const open = async () => {
      ...
      await expect(page.locator("textarea").first()).toHaveValue(record.context.system_prompt);
    };
    await open();
    ...
    await page.reload();
    await open();
    await expect(page.locator("textarea").first()).toHaveValue("Synthetic edited prompt");
    failSave = true;
